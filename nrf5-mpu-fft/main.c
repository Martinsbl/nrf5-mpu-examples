...
void init_buffer(float * buffer)
{
    ...
    {
        *p_buffer = arm_sin_f32(2 * PI * i / 100) + arm_sin_f32(2 * PI * i / 150) ;
        p_buffer++;
    }
}
...
//    }
    ...
    arm_cfft_radix4_instance_f32    radix_instance;
    
    status = arm_rfft_init_f32(&fft_instance, &radix_instance, BUFFER_SIZE, 0, 1);
    arm_rfft_f32(&fft_instance, input_buffer, output_buffer);

    float magn_buffer[BUFFER_SIZE/2];
    arm_cmplx_mag_f32(output_buffer, magn_buffer, BUFFER_SIZE/2);

    float max_value;
    uint32_t max_index;
    arm_max_f32(magn_buffer, BUFFER_SIZE/2, &max_value, &max_index); 
