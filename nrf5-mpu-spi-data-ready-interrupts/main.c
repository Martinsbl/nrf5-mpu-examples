...
#define MPU_MPU_INT_PIN     30 // MPU INT
...
void mpu_setup(void)
{
    ...
    err_code = mpu_init(&m_spi_instance);
...
{
    // Signal to main loop (aka main context) that data is ready to be read from MPU
    mpu_data_ready = true;
}
...
int main(void)
{
    ...
    LEDS_OFF(LEDS_MASK);
    uart_config();
    printf("\033[2J\033[;HMPU example with MPU generated data ready interrupts and SPI transfers. Compiled @ %s\r\n", __TIME__);
    spi_init();
    gpiote_init();
    mpu_setup();
    ...
    while(1)
    {
        nrf_gpio_pin_clear(LED_4); // Pin low when CPU is sleeping. This means that the more the CPU sleeps the brighter LED 4 will shine.
        // Sleep while data is not ready
